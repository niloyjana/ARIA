...
def test_modular_services():
    ...
    b_score = Behavior.calculate_score({"investment_horizon": 15, "reaction_to_market_drop": "buy more", "primary_goal": "wealth growth"})
    print(f"✅ Behavior Service (Aggressive): {b_score}")
    assert b_score == 100
    
    # 2. Test Stability Isolated
    f_score = Stability.calculate_score({"savings_rate": 35, "emergency_fund_months": 8, "total_assets": 1000, "total_liabilities": 100})
